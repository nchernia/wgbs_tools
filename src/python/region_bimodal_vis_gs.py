...
def plot_heatmap_and_hist(read_rows_matrix, fractions_array, out_png, sample_name,
    ...
    if nreads == 0:
        ...
        cmap = matplotlib.colormaps.get_cmap('RdYlBu_r').copy()
...
def main():
    parser = argparse.ArgumentParser(description="Per-region per-read fraction visualization with biallelic detection (supports GCS)")
    parser.add_argument('-r', '--region', required=True, help="Region string chr:start-end")
    parser.add_argument('--glob', default="*.pat.gz", help='Glob pattern (or gs://bucket/path/*.pat.gz for GCS)')
    ...
    elif is_gcs_path(args.glob):
        # Use gsutil to list GCS files
    ...
    # Determine chrom and site-index coordinates
    if args.site_coords:
