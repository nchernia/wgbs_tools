...
def main():
    ...
    # Write header first and flush
    with open(args.output, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        f.flush()
    print(f"Created output file: {args.output}", file=sys.stderr)
    sys.stderr.flush()

    # Checkpoint file for tracking progress
    checkpoint_file = args.output.replace('.csv', '_checkpoint.txt')
    
    for donor_idx, pat_file in enumerate(pat_files, 1):
        donor_name = op.basename(pat_file).replace('.pat.gz', '')
        print(f"\n[{donor_idx}/{len(pat_files)}] Processing {donor_name}...", file=sys.stderr)
        sys.stderr.flush()
        ...
        # Write all results for this donor and flush
        if donor_results:
            with open(args.output, 'a', newline='') as f:
                writer = csv.DictWriter(f, fieldnames=fieldnames)
                writer.writerows(donor_results)
                f.flush()
        
        # Write checkpoint
        with open(checkpoint_file, 'w') as cf:
            cf.write(f"Completed: {donor_idx}/{len(pat_files)} donors\n")
            cf.write(f"Last donor: {donor_name}\n")
            cf.write(f"Total outliers: {total_outliers}\n")
            cf.flush()
